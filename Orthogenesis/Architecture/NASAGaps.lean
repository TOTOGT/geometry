...
-- ═════════════════════════════════════════════════════════════════════════════
--
-- These two gaps were recorded as ∼ partial on the strength of
-- FN_P_101L_schumann_proximity, which re-exported G6Crystal.g6_within_2pct_of_f4:
--
--     |(g6_int : ℝ) - 33.516| / 33.516 < 2 / 100
--
-- That theorem is deleted.  It compared a dimensionless count to a number of
-- hertz, so it was arithmetically true and physically empty; and its subject,
-- passive A₄:₁ coupling to the Schumann n=4 mode, was withdrawn from
-- G6Crystal.lean §4 — the Moon has no Earth–ionosphere cavity to couple to.
--
-- FN_P_402L_noise_tolerance is also removed from this section.  It restated
-- dm3_noise_tolerance (τ·ε₀ = 2/3), a true fact about the dm³ invariants that
-- says nothing about power generation.  Re-exporting a true statement under a
-- gap code does not address the gap; it only makes the gap table look answered.
--
-- The G6 Crystal has no power response.  Both gaps are OPEN, with no partial
-- credit, until a mechanism is proposed that survives its own arithmetic.
...
-- FN-A-104L neighbor traversal — DELETED 2026-08-21 (vacuous).
-- It read:
--     theorem FN_A_104L_neighbor_traversal (h : HexCoord) :
--         ∀ nb ∈ hexNeighbors h, nb ∈ hexNeighbors h := fun nb hnb => hnb
-- That is `x ∈ S → x ∈ S`, discharged by the identity function.  It is true of
-- every set and says nothing about traversal.  Not a `True`-shaped vacuity, so
-- a textual grep for ": True" or ":= trivial" would not have found it — the
-- sweep has to be semantic.  FN-A-104L keeps its real content in
-- FN_A_104L_reachability above, which uses Colony.expandN_mono.
...
-- FN-M-302L  —  REOPENED 2026-08-21 (○ open, no response).
-- It read:
--     theorem FN_M_302L_hex_path_exists :
--         ∀ nb ∈ hexNeighbors ⟨0, 0⟩, nb ∈ hexNeighbors ⟨0, 0⟩ := fun nb hnb => hnb
-- with the docstring "given any two coords in the same colony, there exists a
-- sequence of neighbor steps connecting them".  The statement does not say
-- that.  It says every member of a list is a member of that list, and the
-- proof is `id`.  Unlike FN-A-104L there is no non-vacuous theorem elsewhere
-- carrying this gap, so it is open.
--
-- Closing it honestly needs a reachability relation on HexCoord — an inductive
-- `Path` or the reflexive-transitive closure of hexNeighbors — and a proof that
-- any two coordinates of a connected colony are related by it.  That is real
-- work, not a restatement.  Until then this gap has no response.
