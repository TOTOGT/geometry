...
| N11 | the memristor's pinched hysteresis loop and the corpus's subcritical fold are the same mechanism | *not asserted anywhere* — opened as a question by `book7/ch-van-der-pol.html` | `UNRESOLVED` | memristor hysteresis, pinched loop, subcritical fold, hysteretic setpoint gap | none yet | 2026-09-16 | none — unsearched | **opened as a row, not as a claim.** A memristor's signature is a pinched loop through the origin in the i–v plane; hysteresis in this corpus is the subcritical fold (HVEH Proof III, and the shutdown-vs-startup setpoint gap tagged `[OPEN]` pending measurement on a physical basin). Whether these are one mechanism is an equation-level question. The resemblance is currently at the level of the word *hysteresis*, which is exactly the match this register refuses to accept |
