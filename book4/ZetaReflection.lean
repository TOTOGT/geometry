...
/-
  ...
  WHERE THIS LIVES, AND WHY NOT IN GTCT.  Book 4's Lean is the GTCT repo, and
  its CI ("Verify Lean proofs") does a real kernel check — but it builds only
  `GTCT/GCTC/`, on toolchain v4.11.0, and triggers only on paths under
  `GTCT/GCTC/**`.  This file needs Mathlib v4.32.0 (`Gamma/Digamma.lean` and
  `LSeries_vonMangoldt_eq_deriv_riemannZeta_div`), so putting it there would
  fail that build and turn the badge red.  It lives in the geometry checkout,
  which has the only complete v4.32.0 Mathlib on the machine, until GTCT's
  nested project is moved to 4.32.
