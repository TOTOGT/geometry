...
## THE RULES — read this section; everything below it is reference
...
| R8 | File indexes are generated, never hand-maintained. | `## File indexes` |
| R9 | A duplicate page is declared, or it is a finding. | `## Duplicate pages` |
| R10 | Threshold, not scale — the guard on fold language. | `## Threshold, not scale` |
| R11 | Every declaration named in prose must resolve, at the path cited. | `# RULE (2026-08-27): every declaration` |
| R12 | A published number must be produced by a tool in `tools/`. | `# RULE (2026-08-27): a published number` |
| R13 | What counts as the corpus. | `## What counts as the corpus` |
| R14 | A mismatch is a defect only where something is distributed. | `# RULE (2026-08-28): a mismatch` |
...
Cajueiro-edition Ch 2 (ch2.html, Generative Matrix, off-roster): `Orthogenesis/Matrix/GenerativeMatrix.lean` (2026-09-25, 7). Rank ≤ dim W; dim W < dim V ⇒ C not onto; real vector space with v ≠ 0 is uncountable (V cannot be ℵ₀ and linear); rank-one K update: ≤ r+1, no gain if u ∈ range C, can lower. SVD left/right labels swapped in §2.1 (note only).
Ch 3 circadian (taught path 29, shared by both editions): `Orthogenesis/Circadian/CircadianReeb.lean` (2026-09-25, 12/12 first build). Exercises 3.1–3.2 confirmed (α∧dα = dq∧dp∧dz; R = ∂z unique). Darboux model: Reeb flow z↦z+t, no closed orbit, no attraction (Taubes needs closed M; Reeb orbit is not an attractor). Reeb of cα is R/c; x(c·) period S iff x period cS — doubling rates halves T, contradicting the Falsifiability box. Ex 3.3 Legendrian lift of a loop drops by π per lap.
Ch 4 neural (taught path 30, shared): `Orthogenesis/Neural/NeuralOscillations.lean` (2026-09-25, 12/12 first build). Hopf ż=(λ+iω)z−|z|²z: energy identity, ṙ=λr−r³, decay λ≤0, cycle √λ attracting both sides; no cycle AT λ*=0; θ̇=ω for all λ (40 Hz is input). PLV ∈ [0,1], =1 for any constant difference incl. anti-phase; 0/π samples give 0. Theorem 4.1 info bound not formalised.
Week 6 threshold (31): no math; v2 worked examples (plasma 1.43, figure-eight badges, circadian doubling). Ch 6 resonance (taught path 32, shared): `Orthogenesis/Resonance/ResonanceModes.lean` (2026-09-25, 8/8 first build). Schumann 7.47<c/2πR<7.50, ideal f1 ∈ (10.5,10.7) confirmed. Mode (1,1) no interior node (cross is (2,2)); (1,2)/(2,1) degenerate — one frequency, two patterns; square vs sphere spectra differ (√(5/2) vs √3), Theorem 6.1 "same family" fails.
Ch 7 crystalline (taught path 33, shared): `Orthogenesis/Crystal/CrystallineReturn.lean` (2026-09-25, 15/15 first build; rows by decide, propext only). n-bonacci rows and limit ratios, hexagon least perimeter, |D6|=12, Monster order factorisation, moonshine identities confirmed. Correction: Collatz is not a Millennium Problem; "Millennium Reduction" is not a reduction; Saturn hexagon observed since 1981 (not centuries); CFSG completed 2004; EHT circular.
Ch 8 axiomatic (taught path 34, shared): `Orthogenesis/Axiomatic/AxiomaticTurn.lean` (2026-09-25, 4/4; two axiom-free). Quoted dm3_universality / AXLE/Lexicon/dm3_operators.lean does not exist and is false (ℕ WF, succ no fixed point); true versions: non-increasing on ℕ, Knaster–Tarski; Collatz 3↦10. Gödel chain union still incomplete. Lineage: "son of Foulques V" removed from Letter; "historical and documented" → family tradition, pointing to omega/ch-roger-bacon.html provenance box [OPEN] (ring letter 1265 to nephew Peter Grossi, Potthast; Guido Grossus). Sources disagree on parents (Pierre Foucois/Marguerite Ruffi vs Louis Foulques/Marie Laure Salvanhiac). Career route documented: Raymond VII → Alphonse of Poitiers → Louis IX, enquêteur 1248 (Cahiers de Fanjeaux 57).
Week 10 Lyapunov (taught path 35, shared): `Orthogenesis/Lyapunov/LyapunovHorizon.lean` (2026-09-25, 4/4). Horizon T* and doubling time confirmed; weather paragraph mixes λ=0.9/day (e^9>8000 in 10 days) with daily doubling (λ=ln2, 1024); Lorenz-63 0.906 is dimensionless model time. Theorem 10.1 is a definition.
Week 11 spectral (taught path 36, shared): `Orthogenesis/Spectral/SpectralReach.lean` (2026-09-25, 7/7). Shear: ρ=1 yet unbounded; stretch: ρ=2 yet an orbit →0; herd threshold p>1−1/R₀ (60%, 92–94%); Fibonacci ratios alternate around φ. Theorem 11.1 is a definition.
Week 12 conclusion (taught path 37, shared): `Orthogenesis/Conclusion/FixedPointConclusion.lean` (2026-09-25, 7/7). Banach + a-priori bound + first-revision bound are Mathlib wrappers (stated as such); L≥1 ≠ no fixed point (1.2x fixes 0; id fixes all); 0.5³=1/8, 0.9: 44 steps to 1%. Theorem 12.2 is a definition.
Week 13 revision (taught path 38, shared): `Orthogenesis/Annealing/AnnealingRevision.lean` (2026-09-25, 8/8). Metropolis prob in (0,1], antitone in β, →0; ΔE=0 always accepted; error potential moves minimum (x*−c/2); gradient factor (1−2η), contracts iff 0<η<1. SA global-min only in probability with log schedule (Hajek 1988). Theorem 13.1 definition. Folding note (amyloid) left as text only, unverified.
Week 14 AXLE (taught path 39, shared): `Orthogenesis/Verification/SorryFree.lean` (2026-09-25, 6/6; myProof axiom-free). §1 example compiles; §2/§6 code is pseudocode; 10⁻⁹×3·10⁹=3 errors; Hopfield f₀ⁿ⁺¹ monotone; 2–3 kT → f₀ 0.05–0.14, f₀²>0.0016 not 10⁻⁴ (needs 4.6 kT). Connectome counts inconsistent across ch7/ch14 (FlyWire ~139k).
about-series (40): v2 Lean status section listing all Book 3 Lean files. ch41-first-publication (42): note — contraction gives ONE fixed point for all x₀ (PedagogyDynamics), arrival only in the limit; no new Lean.
D2 academic (43): `Orthogenesis/Epistemology/PythagoreanComma.lean` (3). Tetractys, comma 3¹²/2¹⁹, 23.2–23.7 cents confirmed; note: Issue 6 "verified n ≤ 5" vs AXLE G6Conjecture (X_G6 undefined, range unproved). ch44 (44): addendum — census arithmetic checks; leancheck "error"-name false positive as instrument blind-spot example. Taught path 26–44 complete 2026-09-25 (pre-existing unbalanced <p> in ch-d2-academic left as found).
Cajueiro-edition off-roster chapters: ch3c-econophysics — `Orthogenesis/Market/CircadianTrader.lean` (8/8). T(n) has no clock, non-monotone, negative; bot Λ=|p||Δp+dq| fires at Δp=Δp_prev/2 (z unused); MarketThreshold.lean/bot.py absent; series-DOI mislabel. ch1 (Cajueiro) — `Orthogenesis/Matrix/CajueiroPrinciple.lean` (5): Theorem 1.1 vacuous with unconstrained V_obs (C=0), onto-V impossible, non-injectivity independent of dim; ⌈log₂6·4⌉·3 = 33 arithmetic checked (premises are choices).
g-series — `Orthogenesis/Taxonomy/GSeries.lean` (16): labels g0<g2<g6<g33<g64, total classification regimeOf with partition; 33/64 arithmetic; GCTC `poincare_collatz_contracting` holds for any threshold (33 decorative); GCTC axioms `poincare_collatz` (general) and `inner_basin_is_asymmetric` REFUTED (both false → Chain.lean inconsistent). 33/64 are teaching indices (author, 2026-09-25); Index33Conjecture is a Prop template, no sorry. Open: canonical ladder (GTCT five vs 6·2ᵏ→96), index.html g-series corrections, Figure 2 relabel. See project doc session_2026-09-25_gseries_taxonomy_audit.md.
g-series fold — `Orthogenesis/Taxonomy/FoldHysteresis.lean` (15): cusp slice W=q⁴/4−3q²/2+aq; derivatives proved (HasDerivAt); folds (1,2),(−1,−2); at a=2 equilibria exactly {1,−2}, −2 stable; right branch ⇔ a<2, left ⇔ a>−2; nothing with q≥−2 past a>2; bistable |a|<2 = the irreversible residue. Quasi-static reading is an assumption (flow not formalised). PrincipiaVol1 q³−3q = equilibrium curve, not potential.
g-series CANONICAL (author decision 2026-09-25): two layers. Layer 1 = stages of one fold passage, teaching labels g⁰ seed · g² compositional · g⁶ cycle · g³³ soft equilibrium (W″→0) · g⁶⁴ phase threshold (fold+jump); landing = residue (FoldHysteresis). Layer 2 = levels 1,2,3… (residue seeds next level, new scale/domain), numbered separately. Retired as g-indices: 12, 24, 48, 96 (ch-ladder rungs → levels; chGravity g⁹⁶ → 'g-series at galactic scale', conjecture). ch-ocio transfinite → conjecture. Never call 33/64 derived.
CHAPTER METHOD (author, 2026-09-25): every chapter is one more field run through the same guide — that is why the structure repeats and why students are asked to try. Bridge = translation tool: shared fold normal form + exponents + operators, NOT shared constants/mechanism. Per chapter, besides the Lean round: name state & control; which of C (one slow variable) / K (driver reaches a repeatable threshold) / F (½-power acceleration, 1/√ slowing, jump) / U (hysteresis on reversal) the chapter claims, which it supports, which it skips; status established / model / analogy. See project doc session_2026-09-25_gseries_taxonomy_audit.md.
ch5-immune — `Orthogenesis/Immune/CommitmentThreshold.lean` (6), on FoldHysteresis: C-well ⇔ I<K*=c+2; memory = bistability; irreversible ∀I≥0 ⇔ return fold c−2<0 (5.1's 'irreversible' is about the second fold); bottleneck ∫(r+x²)⁻¹=π/√r ⇒ τ_min ∝ (I−K*)^(−1/2). Operator row: C untested, K yes (moving K*), F model (Martinez PNAS 2012), U established; status model. Corrections: fever is innate not K; 48 h ≈ 16–256 cells; bone-marrow residents are LL plasma cells. Memory labelled F in diagram vs U in §3.
chT-tubulin — `Orthogenesis/Tubulin/TubulinChecks.lean`: κ* 2.81 nm⁻¹ >100× GDP curvature (π/120 nm⁻¹), radius 0.36 nm < monomer; μ_max≠κ* though same definition; β=0.382 is φ⁻² not φ⁻¹; P1 0.92 nm = 3d/13 lattice stagger (not contact form); P2 exp law has no knee. §T.7 Lean excerpt absent from AXLE/geometry; AXLE CatGT/tubulinselectArchitecture.lean defs only (placeholder won't compile); pinned Mathlib has no ContactStructure/Poincaré–Bendixson/contact Darboux. Dynamic instability = stochastic two-state switching, not limit cycle. Operator row: C supported (EB cap, Duellberg eLife 2016), K yes, F untested, U model; status model. Corrections: computronium = Toffoli & Margolus 1991; conservation ≠ optimality; Collatz not Clay, six remain ($6M).
ch9-phi — `Orthogenesis/Phi/SubcriticalApproach.lean`: ratio recursion r(n+1)=1+1/r n; φ fixed point; alternation theorem; contraction factor 1/φ for x≥1; φ exact (1+√5)/2 & irrational; golden-angle forms agree, 137.50°–137.52°; 13/8 row = 0.43% not 0.45%. Operator reading: contraction = U without K or F (the contrast case); 'subcritical' is the opposite in bifurcation theory. Corrections: spiral counts = packing scale not iteration count, not all heads Fibonacci (Swinton 2016); Sollaci & Pereira 2004 is an adoption survey. chT β=0.382 = 2−φ = φ⁻².
ch-ocio — `Orthogenesis/Ocio/OcioChecks.lean`: doubling never settles (iteration ≠ fixed point without contraction); displayed ceiling |P(κ)|>κ¹ is Cantor (κ¹=κ), nothing about GCH; AMonster/GenerativeWeave.lean states 66≠6·11 (false; file outside every target, hyper-Mahlo = ℕ 66). Set theory: GCH consistent with (hyper-)Mahlo (L); ZFC cannot produce inaccessibles (Gödel II); Kanamori table = analogy. AXLE MahloClosure.lean has 3 sorries; ω^ω countable, cof ω, not regular; CatGT copy imports nonexistent Mathlib modules; Crystal.G6 crystal_lockin is sorry. Transfinite rows → conjecture (g-series). Operator: regeneration = basin return, fails past barrier; status analogy. Least action = Maupertuis/Euler/Lagrange; McWhirter 1938.
ch15-entropy — `Orthogenesis/Entropy/EntropyChecks.lean`: merging outcomes cannot raise entropy (H(C(X)) ≤ H(X) step); binary Gibbs D_KL ≥ 0; genome 6e9×1.9/8 = 1.425 GB (chapter's 750 MB = haploid 3e9); V(D)J 2^49<10^15<2^50 (50 bits = max, repertoire non-uniform). Corrections: FlyWire connectome (Nature 2024) proofread not 'formally verified', ~10⁷ bits underived (naive list ~10⁹); §2 optimisation = information bottleneck (Tishby 1999); writing-table H values illustrative; Thm 15.1 is a definition; I = E[KL(posterior‖prior)]. Operator: C only = centre-manifold reduction as information bottleneck; K/F/U not claimed; status analogy.
ch16-scale — `Orthogenesis/Taxonomy/FoldScaling.lean` (general: ẋ=μ+x² invariant under x↦bx, μ↦b²μ, t↦t/b; rest iff μ≤0; √ law; recovery rate 2√d) + `Orthogenesis/Scale/ScaleChecks.lean` (Murray tree dim exactly 3 not 2.97; shrew→whale <8 orders not 10; table ratios 1/100, 1/10, 1/50). Thm 16.1 uncheckable (no R_b on texts); fold = proved RG instance, control relevant exponent 2 — the 'why it recurs' result. Kleiber chart data fit slope 0.73, unsourced; exponent debated (2/3), WBE a model with critiques. Operator: data-collapse test; status analogy.
chΛ-polylaminin — `Orthogenesis/Polylaminin/PolylamininChecks.lean`: P1 threshold = working conc./3 depends on protocol; P2 bound T*·log3 ∈ (T*, 1.2T*). Human pilot now peer-reviewed (Lima…Coelho-Sampaio, Spinal Cord, 2026-08-04; 8 AIS-A, 6→C/D, single-arm, no efficacy claim). Dog study = 6 dogs, TSCIS 2.2→3.2, OFS 1.5→3.1, 6 mo, no control — not 'BBB 4.2→8.8'. Section 2 REWRITTEN (author approved 2026-09-26): SUS history (specialty 1980, PNPIC 2006) + context effects + nanoparticle hypothesis (Chikramane 2010, contamination/replication caveats) + evidence debate (Mathie 2014 low-quality positive; NHMRC 2015, HAS 2019→2021, NHS 2017 negative) + three tests + status list; 'topological mechanism', 'same attractor' and Unified Protocol withdrawn (canvas kept hidden). Intro lead rewritten to match. axle run-protein-model not in AXLE. Operator: polymerisation hysteresis testable (model); clinical recovery not a fold result without controls. ch-tatiana (both copies) updated 2026-09-26: Spinal Cord 2026 paper, Phase 1 authorised 5 Jan 2026 (5 volunteers, ≤72 h), Phase 2 no date.
chH-collatz — `Orthogenesis/Collatz/CollatzChecks.lean`: T5 skeleton's collatz_is_dm3 trivially provable (structure has no map content) ⇒ collatz_convergence_from_dm3 IS Collatz; orbit log-ratios telescope (chapter's own code gives −0.047 / −0.052, not −0.2901 / −0.2878); c/4<1 for odd c iff c∈{1,3}; 3 ∤ (3n+1)/2^k proved. Labels to follow g-series: Step 3, Step 4, Honest Position 'Saturn established' → open/argued. Operator: convergence, not fold (like ch9); status open problem.
chΨ rebuild (2026-09-26, no Lean — empirical claims only): new `chPsi-latent-capacity.html` (acquired savant syndrome → DishBrain/CL1 → FlyWire + Shiu model → computronium; operator table with statuses; APL counterexample; author-account slot left EMPTY — never draft it). `chPsi-quantum-mind.html` kept unchanged under a v2 note: gravity-quantum and mind-across-substrates = holology conjecture (Book 8 ch13); α=dS−p·dq is contact everywhere (can't mark learning), its Reeb field ∂/∂S freezes q. ch14-axle: fly connectome was reconstructed + proofread (FlyWire, Nature 2024, ~139k neurons), not 'formal verification'. AXLE/FruitFly README: no connectome data used; 13 arithmetic theorems real, 3 'obligations' are True/identity placeholders (no sorry keyword, not proved).
ch17-epilogue — `Orthogenesis/Epilogue/EpilogueChecks.lean` (2026-09-26): two distinct fixed points ⇒ not a contraction, so 17.1's Banach claim contradicts the coda's 'depends on x₀' (coda is right; model = FoldHysteresis.bistable, not Banach); 'spectral radius of claims' is a proportion ≤1, =1 when fully evidenced, so 'ρ<1' fails the best paper; L=0.5 to 1% = 7 rounds (3 drafts ⇒ tolerance 1/8). Curry–Howard wording fixed in note; biology = analogy/model not existence proofs. 17.2 vs 17.3 operator→chapter maps disagree (ch 8 in both K and F) — AUTHOR DECIDES which map. No 17.5 heading; prompts numbered 10.x.
overture — `Orthogenesis/Overture/OvertureChecks.lean` (2026-09-26): no finite cardinal is inaccessible (IsInaccessible.nat_lt) ⇒ g⁶=6 not Mahlo; 'hyper-Mahlo from four operators' not provable in ZFC. Memoir untouched and NOT assessed (author's account); read as fold+residue in note. Telos Statement = argued. Monster Regeneration Theorem / Reflection Lemma: no Lean anywhere. Lineage VERIFIED 2026-09-26: Almanac 1792 not first (Poor Richard's 1732–58; first NA almanac 1639); ShadowStats alt-CPI = official + correction (Econbrowser 2008); Yuan–Zheng–Zhu JEF 13 (2006): 48 countries, 1/1973–7/2001, 3–5%/yr, pooled t = −2.44 (page's t≈2.8 corrected in body); Levi epigraph = 'Arsenico', Opere complete I p. 985 (added in body).
prelude (2026-09-26, no Lean — claims about sciences/book): verified: orthogenesis history, Waddington 1940/1942/1957, two-signal T-cell, Taubes Weinstein-3D (G&T 11, 2007). Too strong: clock 'is' Reeb orbit (model; CircadianReeb: Reeb orbit not an attractor); non-abelian anyons in 2DEG = Aug 2026 preprint arXiv:2608.12897 (Heiblum/Sim), engineered anyons Google Nature 2023 / Quantinuum Nature 2024; sleep 'do not consolidate' → less well; KaiABC ≠ TTFL (Nakajima 2005); Chladni≠lattices. No source: Mandelbrot tiles K-F-C boundary. AUTHOR DECIDES: operator set (page defines C,K,F,T,U,G; U=Universal vs Unfolding), ch7/ch8 references vs nav, 17 ch vs 16 weeks vs 12+Epilogue, Epilogue 'CEFR D2' not a CEFR level.
Sportal (2026-09-26, no Lean): AUTHOR: portal is an EXAMPLE, not full coverage — note adds a generic 'point the LLM at a chapter link + ask for claims/statuses/what it couldn't check' prompt. Verified: CEFR A1–C2 (D1/D2 series stages; C2 skipped); Cambridge A2 = 180–200 GLH (not '21 days'); 33 = teaching index (index33_fails_double); B2.2 lemniscate ≠ analemma (analemma_not_gerono); Θ=33+N×M: AXLE header says conjecture, only arithmetic checked; χ(H*(X⁶))=33 ill-posed (X⁶, n undefined); ISBN 979-8-9954416-0-1 check digit valid. Generic prompt also requires §-numbers per claim (else 'my addition'), ends on a section to reread + Lean file, and forbids recommending other platforms — author: LLMs otherwise keep students on their own platforms. Prompts unchanged.
index.html g-series section (2026-09-26): 'formally verified, zero axioms' → teaching taxonomy with proved fold core (FoldHysteresis, FoldScaling); Chain.lean's false axioms refuted (GSeries); poincare_collatz_contracting gives 33 no role; T1 not proved (gtct_t1 assumes its conclusion); 'helical attractor locks' removed; dorsal-striatum line softened (no study ties to 33); Pirangi planted 1888 (was '70 years'), 8,500 m² ≈ 2 acres; points to g-series.html.
Vol I next edition — "For the Holy Father" letter fixes (owner approved 2026-09-25): replace "He was also of the Templars" with "It was his forebear by tradition, Fulk V, who first endowed the Templars, in 1120. As pope, Clement was the order's direct superior, as every pope had been since 1139." Also: Summa left unfinished (Dec 1273), not "completed 1274"; Chinon Parchment found 2001, published 2007, absolution 1308; tomb moved from Santa Maria in Gradi to San Francesco alla Rocca in the 1880s (not "since 1268"); birth c. 1190.
