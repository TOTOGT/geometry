/-
  ...
  STATUS, 2026-09-13: kernel-audited under the v4.32.0 pin. Three declarations
          — sin_cube, spectral_gap, fold_locus — on [propext, Classical.choice,
          Quot.sound] and nothing else. Report:
          tools/verify-audit/2026-09-13/geometry__ReactionDiffusionFold.axioms.txt
          A hand run dates from the day it was run, so the file is a declared
          build target from the same date.

          WHAT THE THREE COVER, AND WHAT THEY DO NOT. They are the algebraic and
          spectral core: the cubic mode-coupling identity, the spectral gap, and
          the fold-locus discriminant. The center-manifold / inertial-manifold
          *existence* result and the O(a⁵) asymptotic feedback bound are standard
          theorems, invoked and not formalized here. The reduction is rigorous
          modulo those two, and the chapter says so.
